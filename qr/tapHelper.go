...
func LoginQrCode(useChinaEndpoint bool, permissions ...string) (r *LoginQrCodeRes, err error) {
	...
	writer.WriteField("client_id", phigros.ClientId)
...
func GetProfile(useChinaEndpoint bool, c *CheckQRCodeRes) (p *Profile, err error) {
	...
	var urlt string
	if useChinaEndpoint {
		urlt = ChinaApiHost + "/account/profile/v1?client_id=" + phigros.ClientId
	} else {
		urlt = ApiHost + "/account/profile/v1?client_id=" + phigros.ClientId
	}
